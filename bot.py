...
Версия: 28.0 (FINAL - ТОЛЬКО ПЕРЕМЕННЫЕ ОКРУЖЕНИЯ)
Функции:
...
if not BOT_TOKEN:
    ...
    logger.error("📝 Инструкция: https://dashboard.render.com/web/srv-d6au598gjchc73bqb88g/env")
    sys.exit(1)

logger.info(f"✅ Токен бота успешно загружен из переменных окружения: {BOT_TOKEN[:10]}...")
...
# ================= ПРОСТОЙ ВЕБ-СЕРВЕР В ОТДЕЛЬНОМ ПОТОКЕ =================
# Это гарантирует, что Render увидит открытый порт

async def simple_handle(request):
...
async def run_simple_server():
    ...
    app = web.Application()
    app.router.add_get('/', simple_handle)
    app.router.add_get('/health', simple_health)
    ...
    logger.info(f"✅ Простой веб-сервер запущен на порту {PORT}")
...
def start_simple_server():
    ...
    try:
        loop.run_until_complete(run_simple_server())
    except Exception as e:
        logger.error(f"❌ Ошибка в веб-сервере: {e}")
    finally:
        loop.close()

# Запускаем веб-сервер в отдельном потоке (сразу, до всего)
...
if __name__ == "__main__":
    print("=" * 80)
    print("🚀 Telegram Numbers Shop Bot v28.0 - ТОКЕН В ПЕРЕМЕННЫХ ОКРУЖЕНИЯ")
